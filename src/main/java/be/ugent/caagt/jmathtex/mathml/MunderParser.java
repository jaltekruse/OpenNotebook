...
class MunderParser extends PresentationElementParser {

   private static final String ACCENT = "accentunder";
   ...
   public TeXFormula buildFormula(Element el, Environment env)
      ...
      if (formulas.size() != 2)
         throw new MathMLException("munder needs exactly 2 child elements!");
      ...
      if (accent != null && accent.equals("true"))
         ...
         return ((TeXFormula) formulas.get(0)).putUnderAndOver(
               (TeXFormula) formulas.get(1), ACC_LIM_UNIT, ACC_SPACE, false, null,
               ACC_LIM_UNIT, 0, false);
      else if (accent == null || accent.equals("false")) // default
         return new TeXFormula().addOp((TeXFormula) formulas.get(0),
               (TeXFormula) formulas.get(1), null, true);
      else
         throw new MathMLException("Invalid attribute value for 'accent' : '"
               + accent + "'!");
   }
}
